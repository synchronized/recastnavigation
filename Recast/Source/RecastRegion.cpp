								  ...
								  unsigned short* srcReg, rcIntArray& overlaps)
{
	...
	for (int i = 0; i < nreg; ++i)
	{
		...
		// 如果累积区域大小太小，请将其删除。 不要删除连接到图块边框的区域，
		// 因为无法正确估计它们的大小，并且删除它们可能会删除必要的区域。
	...
	do
	{
		...
		for (int i = 0; i < nreg; ++i)
		{
			...
			// 具有超过 1 个连接的小区域。 或者根本不与边界相连的区域。 找到连接到该区域的最小相邻区域。
...
///
/// 这通常是创建完全构建的紧凑高度场的倒数第二步。 在使用 #rcBuildRegions 或
/// #rcBuildRegionsMonotone 构建区域之前需要执行此步骤。
...
/// 如果多个区域形成一个小于@p minRegionArea 的区域，则所有跨度将被重新分配给零（空）区域。
///
/// Partitioning can result in smaller than necessary regions. @p mergeRegionArea helps
/// reduce unnecessarily small regions.
/// 分区可能会导致区域小于必要的区域。 @p mergeRegionArea 有助于减少不必要的小区域。
///
...
/// 区域数据可通过 rcCompactHeightfield::maxRegions 和 rcCompactSpan::reg 字段获得。
///
/// @warning The distance field must be created using #rcBuildDistanceField before attempting to build regions.
/// @警告在尝试构建区域之前，必须使用#rcBuildDistanceField 创建距离字段。
///
...
/// 流域分区可能会导致区域小于必要的区域，特别是在对角走廊中。 @p mergeRegionArea 有助于减少不必要的小区域。
///
/// See the #rcConfig documentation for more information on the configuration parameters.
/// 有关配置参数的更多信息，请参阅#rcConfig 文档。
///
					...
					const int borderSize, const int minRegionArea, const int mergeRegionArea)
{
	...
	// TODO：绘制更好的公式，expandIters 定义分水岭“溢出”的程度并简化区域。
	// 将它与代理半径联系起来通常可以很好地表明它的贪婪程度。
